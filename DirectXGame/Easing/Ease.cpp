...
Vector2 Ease::EaseInMove(Vector2& OwnPos, Vector2& tmp1, Vector2& tmp2, int& nowFrame, int& maxFrame) {
	float easeT = float(nowFrame) / maxFrame;

	easeT *= easeT;

	OwnPos.x = (1.0f - easeT) * tmp1.x + easeT * tmp2.x;
	OwnPos.y = (1.0f - easeT) * tmp1.y + easeT * tmp2.y;

	return OwnPos;
}

Vector2 Ease::EaseOutMove(Vector2& OwnPos, Vector2& tmp1, Vector2& tmp2, int& nowFrame, int& maxFrame) {
	float t = float(nowFrame) / maxFrame;
